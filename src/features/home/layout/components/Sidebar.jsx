    ...
    return (
        ...
        >
            <Link
                ...
                className={`
                    group
                    flex ${isCollapsed ? 'justify-center' : 'flex-row gap-3'} items-center
                    ${isCollapsed ? 'p-2' : 'px-4 py-2'} rounded-2xl ${isCollapsed ? '' : 'w-full'}
                    transition-colors
                    ${active ? 'bg-blue-800' : 'hover:bg-blue-100'}
                `}
                title={isCollapsed ? item.label : undefined}
            >
                <div className={`flex justify-center items-center w-10 h-10 ${active ? 'text-white' : 'text-gray-900 group-hover:text-blue-800'}`}>
                    <Icon />
                </div>
                {!isCollapsed && (
            ...
            {isCollapsed && hoveredIndex === index && !active && (
                <div className="absolute left-[67px] top-1/2 -translate-y-1/2 z-50 pointer-events-none animate-[fadeIn_0.15s_ease-in-out]">
        ...
        ">
            <div className="flex flex-col justify-between items-center h-full w-full">
                <div className="flex flex-col justify-center items-center gap-[14px] w-full">
                    {/* Logo */}
                    <div className="w-full px-10 py-3 flex justify-start items-center">
                        <img
                            src={getLogoUrl()}
                            alt="Logo"
                        />
