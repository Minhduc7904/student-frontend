...
import {
    ...
    selectNotificationStatsFetched,
...
const BellButton = memo(({ unread, onClick, compact = false }) => (
    <button
        onClick={onClick}
        className={`h-8 relative inline-flex items-center justify-center rounded-lg transition-all duration-200 cursor-pointer active:scale-90 hover:bg-gray-100 ${compact ? 'p-1.5' : 'p-1'}`}
        aria-label="Thông báo"
    >
        <Bell size={compact ? 18 : 20} className="text-gray-800" />
        {unread > 0 && (
            <span
                className={`absolute rounded-full bg-red-500 ring-2 ring-white ${compact ? 'top-0 right-0 h-2 w-2' : '-top-0.5 -right-0.5 h-2.5 w-2.5'}`}
                aria-hidden="true"
            />
        )}
    </button>
...
const NotificationBell = memo(({ compact = false }) => {
    ...
    const statsFetched = useSelector(selectNotificationStatsFetched);
    ...
    useEffect(() => {
        ...
        if (!myNotificationsFetched) {
            dispatch(getMyNotificationsAsync({ page: 1, limit: 10 }));
        }
    }, [dispatch, myNotificationsFetched, statsFetched]);
